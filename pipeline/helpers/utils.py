...
class MLUtils(ConfigurationBuilder):
    ...
    def preprocess_data(self, df: pd.DataFrame, target_column: str = 'Close', look_back: int = 30, test_size: float = 0.2):
        """
        ...
        feature_columns = [col for col in df.columns if col not in ['Open time', target_column]]
        ...
        n_train = int((1 - test_size) * len(df))  # Index where the train-test split occurs
