...
class DBFuncsETL(ConfigurationBuilder):
    """
        ...
        This class provides methods for managing and manipulating ClickHouse tables, including creating tables,
        dropping tables, removing duplicates, and other data-related operations. It extends the `ConfigurationBuilder`
        class to incorporate configuration management functionality and is designed to streamline the process of
        maintaining database tables and ensuring data integrity.

        Inherits:
            ConfigurationBuilder: Provides configuration management methods used to set up and manage database connections.
    ...
    __tech_db = {'INFORMATION_SCHEMA', 'default', 'information_schema', 'system'}


    def get_tablename(self):
        """
            Generate and return the table name based on the current database, currency, and interval.

            This method constructs the table name using the format:
            "{database}.{currency}_{interval}", where `database`, `currency`, and `interval`
            are attributes of the current instance.
    ...
    def create_table(self, suffix=''):
        ...
        Raises:
            Exception: If all retry attempts fail, the last exception is raised.
        """
        table_schema = ', '.join(f'{fieldname} {datatype}' for fieldname, datatype in self.table_columns)
    ...
    def remove_duplicates(self):
        ...
        columns = ",\n".join(
            [f"any({col_name}) as {col_name}" for col_name, _ in self.table_columns if col_name != 'Open_time'])
